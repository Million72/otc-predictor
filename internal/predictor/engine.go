...
func (e *Engine) Predict(market string, duration int) (types.Prediction, error) {
	...
	// (Removed: a flat +5% confidence multiplier used to be applied here
	// whenever tick volume was abundant. It wasn't based on any measure
	// of actual accuracy, just data quantity, so it silently overstated
	// confidence on every prediction with enough ticks. Confidence now
	// reflects only what the strategy layer actually computed.)// Cache it with longer timeout for longer durations
