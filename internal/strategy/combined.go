...
func (s *CombinedStrategy) marketAwareConsensus(signals []types.StrategySignal, basePrediction types.Prediction, marketType string) types.Prediction {
	...
	for _, signal := range signals {
		if signal.Direction == "UP" {
			...
			upNames = append(upNames, signal.Name)
			...
			downNames = append(downNames, signal.Name)
		}
	}
	...
	// Respect the configured threshold (strategy.min_confidence in
	// config.yaml) instead of a hardcoded value — falls back to a safe
	// default only if the config left it unset.
	...
	basePrediction.Signals = finalNames
